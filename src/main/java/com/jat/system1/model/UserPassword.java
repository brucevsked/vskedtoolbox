...
public class UserPassword {
    ...
    public UserPassword(String password) {
        ...
        if("".equals(trimPassword)){
            throw new IllegalArgumentException("用户密码不能为空字符串！");
        }

        if(password.length()<minLength){
            throw new IllegalArgumentException("用户密码长度过短！长度应为"+minLength+"～"+maxLength+"个字符");
        }

        if(password.length()>maxLength){
            throw new IllegalArgumentException("用户密码长度过长！长度应为"+minLength+"～"+maxLength+"个字符");
        }
